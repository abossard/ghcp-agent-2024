(function () {
  ...
  exportBtn?.addEventListener("click", () => {
    fetch("/api/owners/export", { method: "GET" })
      ...
      .catch((err) => {
        reportEl.textContent = err.toString();
      });
  });
  ...
  importBtn?.addEventListener("click", () => {
    ...
    if (!f) {
      reportEl.textContent = "Please choose a file first.";
      return;
    }
  ...
  importApplyBtn?.addEventListener("click", () => {
    if (
      !lastReport ||
      !confirm("Are you sure you want to apply the validated import?")
    )
      return;
